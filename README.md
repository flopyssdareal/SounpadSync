...
- `ffmpeg.exe` и `ffprobe.exe` (скачать билды можно [здесь](https://www.leppsoft.com/soundpad/))
...
Если Soundpad запущен от имени администратора — SoundpadSync тоже нужно запускать от администратора (иначе Windows не даст открыть канал между процессами с разными правами).
...
**"Pipe not found / Error 2"** — Soundpad не запущен, либо приложение запущено без прав администратора, а Soundpad — с ними (см. раздел «Настройка Soundpad» выше).
